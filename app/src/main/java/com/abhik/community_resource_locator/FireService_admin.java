...
public class FireService_admin extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fire_service_admin);
        ...
        chooseMap.setOnClickListener(v -> {
            ...
            if (name.isEmpty()) {
                Toast.makeText(FireService_admin.this, "Please enter a name first", Toast.LENGTH_SHORT).show();
                return;
            }
            Intent intent = new Intent(FireService_admin.this, On_AmbulanceAdmin.class);
    ...
    private void saveDataToFirebase() {
        ...
        if (name.isEmpty() || phone.isEmpty() || email.isEmpty() || address.isEmpty() || vehicleCount.isEmpty() || serviceHours.isEmpty()) {
            Toast.makeText(FireService_admin.this, "Please fill all fields", Toast.LENGTH_SHORT).show();
            return;
        }

        DatabaseReference userRef = databaseReference.child("FireServices").child(name);
        ...
        userRef.setValue(dataMap).addOnSuccessListener(aVoid -> {
            ...
            userRef.child("location").setValue(locationMap).addOnSuccessListener(aVoid1 -> {
                Toast.makeText(FireService_admin.this, "Data saved successfully!", Toast.LENGTH_SHORT).show();
                finish(); // Return to the previous activity
            }).addOnFailureListener(e ->
                    Toast.makeText(FireService_admin.this, "Failed to save location", Toast.LENGTH_SHORT).show()
            );
        }).addOnFailureListener(e ->
                Toast.makeText(FireService_admin.this, "Failed to save data", Toast.LENGTH_SHORT).show()
        );
    }
